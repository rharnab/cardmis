...
$ch_query = mysql_query("SELECT payable_amount, sl,  paid_fee_amt, due_fee_amt, account_no, card_no, usd_rate   FROM auto_debit_charge  WHERE account_no <> '' and due_fee_amt <> '0' and status <> 1 and exc_till_date >= '$today'");
				...
				if($error =='N')
				{
				 ...
				 if($balance > $payable_amount)
				 {
					...
					if($balance >= $due_fee_amt)
					{
						...
						$api_data = chargeBalance($account_no_file, $log_paid, $debit_sl, $payable_amount, $usd_rate);
