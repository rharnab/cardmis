...
$ch_query = mysql_query("SELECT card_fee, vat, sl, total_card_fee, paid_fee_amt, due_fee_amt, account_no_file, card_no_file,payable_net_amount, payable_vat_amount, narrition,debit_amount   FROM debit_charge_deduction  WHERE account_no_file <> '' and due_fee_amt <> '0' and status <> 1 order by sl desc");
