...
$accounts = implode(',', str_replace(' ','' , $account_array_list));
    ...
    if($_SESSION['branch_id'] =='1')
    {
       ...
       $update_aprrove =  mysql_query("UPDATE debit_card_api set card_status='$status', approve_by_id='$approve_id', approve_date='$approveDate', batch_num='$fb_num', status='2' where accountno in ($accounts) and request_type='2' ");
     ...
     if($update_aprrove)
      {
    	...
    	$update_aprrove= mysql_query("UPDATE debit_card_api set status='1' where accountno in ($accounts) and request_type='2' ");
