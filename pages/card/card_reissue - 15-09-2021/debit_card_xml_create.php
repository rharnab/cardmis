<?php
	...
	$sl_debit_card_api = $conn->prepare("SELECT * FROM debit_card_api WHERE card_status !='dishonour' AND  card_status ='approve' and status = '2' and request_type='2' and accounttype='$acc_type' ");
	...
	$select = $conn->prepare("SELECT * FROM debit_card_api WHERE request_type='2' and card_status !='dishonour' AND card_status ='approve' and status = '2' and accounttype='$acc_type'  ");
	$select->execute();
	while($row = $select->fetch(PDO::FETCH_OBJ)){
		...
		if($insert_backup->execute()){
			...
			$delete_api_data = $conn->prepare("UPDATE  debit_card_api set status='3'  WHERE request_type='2' and accounttype='$acc_type' and id=?");
