...
if(mysql_num_rows($sql) > 0)
{
	...
	while($result = mysql_fetch_array($sql))
	{
		...
		$on_card_no =$result['card_no'];
		$name_on_card = '<input type="hidden" name="" class="card_name" value="'.$on_card.'">';

		if(trim($result['card_status']) == 'Given' || trim($result['card_status']) == 'Expired')
		{
			$card_no = '<input type="hidden" name="" class="card_no" value="'.$on_card_no.'">';
		}else{
			$card_no = '';
		}
		...
		$table .= '<tr>';
			$table.= '<td>'.$sl++.'</td>';
			$table.= '<td>'.$result['card_no'].$card_no.'</td>';
