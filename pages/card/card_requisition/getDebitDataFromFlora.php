...
<?php
	...
	$accfather = $_POST['father_name'];
	$mothers_name = $_POST['mother_name'];
	$accsex = $_POST['gender'];
	$accdateofbirth = $_POST['birth_dt'];
	$marketed_by = $_POST['marketed_by'];


	//type 
	$request_type = '1';// for new card
	...
	//duplicate check
	$dup_sql= mysql_query("SELECT accountno from debit_card_api where   accountno='$accNumber' and request_type='1' and status <> '5' and branch_id <> 0 ");
	...
	if(empty($api_data)){
		...
		$accnameoncard = mysql_real_escape_string($noc);
		$accopendate = $api_data['opend'];
		$accfather = mysql_real_escape_string($accfather);
		...
		$mothers_name = mysql_real_escape_string($mothers_name);
		$entry_by_id = $user_id;
		



		$debit_insert = mysql_query(" INSERT INTO debit_card_api (accountno, customerid, accounttype, actypecode, accountname, accstatus, accnameoncard, accopendate, accfather, nationalid, accphone, accotheremail, accotherphone, accaddress, accdateofbirth,accsex, bal_tk, entry_by_id, requestDate, branch_id, status, receive_branch, request_type, issue_branch, narration, accmother, markated_by) VALUES ('$acc_no', '$customerid', '$accounttype', '$actypecode', '$accountname', '$accstatus', '$accnameoncard', '$accopendate', '$accfather', '$nationalid', '$accphone', '$accotheremail', '$accotherphone', '$accaddress', '$accdateofbirth', '$accsex', '$bal_tk','$entry_by_id', '$currentDate', '$branch_id', '0', '$rcv_branch', '$request_type', '$issue_branch', '$narration', '$mothers_name', '$marketed_by') ");

		$sql_t = "INSERT INTO debit_card_api (accountno, customerid, accounttype, actypecode, accountname, accstatus, accnameoncard, accopendate, accfather, nationalid, accphone, accotheremail, accotherphone, accaddress, accdateofbirth,accsex, bal_tk, entry_by_id, requestDate, branch_id, status, receive_branch, request_type, issue_branch, narration, accmother, markated_by) VALUES ('$acc_no', '$customerid', '$accounttype', '$actypecode', '$accountname', '$accstatus', '$accnameoncard', '$accopendate', '$accfather', '$nationalid', '$accphone', '$accotheremail', '$accotherphone', '$accaddress', '$accdateofbirth', '$accsex', '$bal_tk','$entry_by_id', '$currentDate', '$branch_id', '0', '$rcv_branch', '$request_type', '$issue_branch', '$narration', '$mothers_name', '$marketed_by')";


		file_put_contents('r.txt',$sql_t.PHP_EOL, FILE_APPEND);

		//file_put_contents('ramjan.txt', $sql_t);
