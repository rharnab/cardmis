...
function re_issue_charge_fee()
{
	$charge = 300;
	$vat =    45;
	return $re_charge = array('charge_fee'=> $charge, 'vat' => $vat);
}

/*pin re-issue charge*/
function pin_re_issue_charge_fee()
{
	$charge = 150;
	$vat =    22.50;
	return $re_charge = array('charge_fee'=> $charge, 'vat' => $vat);
...
function balanceCheck($acc_no, $debit_sl)
{
	...
	$insert_cvs_log  = mysql_query("INSERT INTO re_issue_cbs_log (log_id, error, cvs_log, log_type, logTimeStamp ) values ('$debit_sl', '$error', '$acc_response', '0', '$currentTimeStamp') ");

	return $balance_data = array('error'=> $error, 'balance'=>$balance);
}

function chargeBalance($acc_no, $pId_amt, $debit_sl, $vat, $card_fee, $narration)
{
	...
	// Multiple Transaction
	   ...
	   $bnk_response = mysql_real_escape_string($bnk_response);
	  	file_put_contents('rr.txt', 'p-'.$pId_amt.'c-'.$card_fee.'v-'.$vat.PHP_EOL, FILE_APPEND);
		//store log for caharge apply
			$insert_cvs_log  = mysql_query("INSERT INTO re_issue_cbs_log (log_id, error, cvs_log, log_type, logTimeStamp ) values ('$debit_sl', '$bnk_error', '$bnk_response', '1', '$currentTimeStamp') ");
