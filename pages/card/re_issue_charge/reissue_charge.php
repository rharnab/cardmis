...
$ch_query = mysql_query(" SELECT card_fee, vat, sl, total_card_fee, paid_fee_amt, due_fee_amt, account_no_file, card_no_file,payable_net_amount, payable_vat_amount, narrition,debit_amount, request_type   FROM re_issue_charge_deduction  WHERE account_no_file <> '' and due_fee_amt <> '0' and status <> 1 order by sl ");
while($ch_result = mysql_fetch_array($ch_query))
{
	$total_card_fee = $ch_result['payable_net_amount'];
	...
	$vat = $ch_result['payable_vat_amount'];
	$card_fee = $ch_result['debit_amount'];
	$narration = $ch_result['narrition'];
	$request_type = $ch_result['request_type'];
				...
				if($error =='N')
				{
				 ...
				 if($balance > $total_card_fee)
				 {
					...
					if($balance >= $due_fee_amt)
					{
						...
						$api_data = chargeBalance($account_no_file, $log_paid, $debit_sl, $vat, $card_fee, $narration);
						...
						if($bnk_error == 'N')
						{
							...
							$charge_update = mysql_query("UPDATE re_issue_charge_deduction set paid_fee_amt = '$paid_fee_amt', due_fee_amt='$due_fee_amt', status='$sts' where sl='$debit_sl' ");
							...
							if($charge_update)
							{
								...
								$log_query = mysql_query("INSERT into re_issue_charge_log (card_no, acc_no, balance_amt, paid_amt, due_amt, charge_date, msg,batch_no,TraceNoList_0, TraceNoList_1, debit_sl, entry_date, card_fee, vat) values ('$card_no_file', '$account_no_file',  '$log_balance', '$log_paid', '$log_due', '$today', '$msg', '$batch_no', '$TraceNoList_0', '$TraceNoList_1',  '$debit_sl', '$today', '$card_fee', '$vat') ");
